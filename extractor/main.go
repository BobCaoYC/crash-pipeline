...
var (
	extractorStartTime = prometheus.NewGauge(prometheus.GaugeOpts{
		Name: "extractor_start_time_seconds",
		Help: "Unix timestamp when the extractor service started.",
	})

	extractorRunsTotal = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "extractor_runs_total",
			Help: "Total number of extractor jobs by status.",
		},
		[]string{"status"}, // started | success | failure
	)

	extractorErrorsTotal = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "extractor_errors_total",
			Help: "Total number of extractor errors by type.",
		},
		[]string{"type"}, // e.g. process_job, http_crashes, http_enrich
	)

	extractorRunDuration = prometheus.NewHistogram(
		prometheus.HistogramOpts{
			Name:    "extractor_run_duration_seconds",
			Help:    "Duration of each extractor job in seconds.",
			Buckets: prometheus.DefBuckets,
		},
	)

	extractorRowsProcessed = prometheus.NewCounter(
		prometheus.CounterOpts{
			Name: "extractor_rows_processed_total",
			Help: "Total number of rows fetched and written by the extractor (crash + enrich).",
		},
	)

	extractorStageDuration = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Name:    "extractor_stage_duration_seconds",
			Help:    "Duration of major extractor stages.",
			Buckets: prometheus.DefBuckets,
		},
		[]string{"stage"}, // fetch_crashes_page, fetch_enrich_page, write_object, etc.
	)
)
...
func init() {
	prometheus.MustRegister(
		extractorStartTime,
		extractorRunsTotal,
		extractorErrorsTotal,
		extractorRunDuration,
		extractorRowsProcessed,
		extractorStageDuration,
	)
}
	...
	for {
		...
		for y, recs := range buckets {
			...
			if err := putJSONGZ(mcli, env, key, b, meta); err != nil {
				extractorErrorsTotal.WithLabelValues("write_crashes").Inc()
				return "", false, err
			}
			extractorStageDuration.WithLabelValues("write_object").Observe(time.Since(tWrite).Seconds())
			extractorRowsProcessed.Add(float64(len(recs)))
...
func fetchCrashesPage(env Env, job Job, offset int) (rows int, crashIDs []string, body []byte, maxUpdated time.Time, err error) {
	stageStart := time.Now()
	defer func() {
		extractorStageDuration.WithLabelValues("fetch_crashes_page").Observe(time.Since(stageStart).Seconds())
	}()
...
func fetchOneBatchAllPages(env Env, mcli *minio.Client, job Job, corr string, ds *DatasetSpec,
	...
	for off := 0; ; off += limit {
		...
		// Ensure join key is present; don't force crash_date here (dataset may not have it)
		...
		// --- metrics: fetch_enrich_page ---
		tFetch := time.Now()
		raw, err := httpGetJSON(env, u)
		extractorStageDuration.WithLabelValues("fetch_enrich_page").Observe(time.Since(tFetch).Seconds())
		if err != nil {
			log.Printf("enrich %s batch=%d off=%d: %v", ds.Alias, batchIdx, off, err)
			extractorErrorsTotal.WithLabelValues("http_enrich").Inc()
			return
		}
		...
		if err := json.Unmarshal(raw, &arr); err != nil {
			...
			extractorErrorsTotal.WithLabelValues("parse_enrich").Inc()
			return
		}
		rows := len(arr)
		if rows == 0 {
		...
		// Group records by year using crashYear map; fallback to row's own crash_date if present
		...
		for y, recs := range groups {
			...
			if y == 0 { // avoid year=0000 folders
			...
			// --- metrics: write_object + rows processed for enrich ---
			tWrite := time.Now()
			if err := putJSONGZ(mcli, env, key, payload, meta); err != nil {
				log.Printf("save %s: %v", key, err)
				extractorErrorsTotal.WithLabelValues("write_enrich").Inc()
			} else {
				extractorStageDuration.WithLabelValues("write_object").Observe(time.Since(tWrite).Seconds())
				extractorRowsProcessed.Add(float64(len(recs)))
				log.Printf("saved: s3://%s/%s (year=%d, rows=%d)", env.RawBucket, key, y, len(recs))
...
func main() {
	...
	metricsAddr := getenv("METRICS_ADDR", ":2112") // env override if you like
	extractorStartTime.SetToCurrentTime()
	...
	for d := range msgs {
		jobStart := time.Now()
		extractorRunsTotal.WithLabelValues("started").Inc()

		var job Job
		if err := json.Unmarshal(d.Body, &job); err != nil {
			log.Printf("bad job json: %v", err)
			extractorErrorsTotal.WithLabelValues("bad_job_json").Inc()
			extractorRunDuration.Observe(time.Since(jobStart).Seconds())
			extractorRunsTotal.WithLabelValues("failure").Inc()
			d.Ack(false)
			continue
		}
		if job.Primary.ID == "" {
			log.Printf("job missing primary.id")
			extractorErrorsTotal.WithLabelValues("missing_primary_id").Inc()
			extractorRunDuration.Observe(time.Since(jobStart).Seconds())
			extractorRunsTotal.WithLabelValues("failure").Inc()
			d.Ack(false)
			continue
		...
		extractorRunDuration.Observe(duration)

		if err != nil {
			log.Printf("job failed: %v", err)
			extractorErrorsTotal.WithLabelValues("process_job").Inc()
			extractorRunsTotal.WithLabelValues("failure").Inc()
		} else {
			log.Printf("job done in %s (corr=%s, wroteAny=%v)", time.Since(jobStart), corr, wroteAny)
			extractorRunsTotal.WithLabelValues("success").Inc()
		...
		if wroteAny && corr != "" {
			...
			if err := publishTransformJob(env.RabbitURL, tj); err != nil {
				...
				extractorErrorsTotal.WithLabelValues("publish_transform_job").Inc()
			} else {
